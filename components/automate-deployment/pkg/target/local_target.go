...
func (t *LocalTarget) HabSupRestart(sortedServiceList []string) (bool, error) {
	...
	return t.SystemdRestartWithSupVersion(habSupP, sortedServiceList)
}
...
func (t *LocalTarget) SystemdRestart(sortedServiceList []string) (bool, error) {
	habSupP, err := t.habSup.SupPkg()
	if err != nil {
		return false, errors.Wrap(err, "determining running hab-sup version")
	}
...
func (t *LocalTarget) SystemdRestartWithSupVersion(habSupP habpkg.HabPkg, sortedServiceList []string) (bool, error) {
	...
	for i := len(sortedServiceList) - 1; i >= 0; i-- {
		...
		err := t.unloadServiceWithHabVersion(&svc, binPkg, habSupP)
		if err != nil {
			logrus.WithFields(logrus.Fields{
